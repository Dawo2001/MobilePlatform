...
RTMP_URL = "rtmp://192.168.68.102/live/rgb"
...
def run():
    ...
    while loop.loop_again:
        if not c.isOpened():
            print("Could not connect to RTMP.")
            exit()
        else:
            print("Connected to RTMP.")

        while loop.loop_again:
            ret, frame = c.read()
            if not ret:
                print("Frame could not be opened.")
                pass
            else:
                ...
                output, distance = measuring_disc_detection.detect_disc(img)
                cv2.imshow('RTMP Stream', output)
                if licznik == 0:
                    data = pid.compute(distance)
                    print(data)
                    print(distance)
                    send_data = data + (0.0,) * (   6)
                    byte_data = struct.pack('!%sf' % len(send_data), *send_data)
                    sc.send(byte_data)
                    licznik = limit_licznika
                else:
                    licznik -= 1
